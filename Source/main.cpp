...
public:
    ...
    inline ~CyclicTimer();
    inline unsigned long int get_timeout_tick(std::chrono::system_clock::time_point now) const;
    inline void start(unsigned int interval_ms, std::function<void()> callback);
    inline void stop();
};
inline CyclicTimer::CyclicTimer()
  : running(false)
{
}
inline CyclicTimer::~CyclicTimer()
...
inline unsigned long int CyclicTimer::get_timeout_tick(std::chrono::system_clock::time_point now) const
{
...
inline void CyclicTimer::start(unsigned int interval_ms, std::function<void()> callback)
{
...
public:
    ...
    std::list<std::string> split(std::string & src);
};
...
inline void BinaryControl::set_pos(size_t pos)
{
    this->pos = pos;
}
...
static mrb_value mrb_cppregexp_reg_grep(mrb_state* mrb, mrb_value self);
...
static mrb_value mrb_cppregexp_reg_select(mrb_state* mrb, mrb_value self);
...
/* -- static tables -- */
static const char *  SoftwareRevision = "0.15.03";
...
public:
    ...
    void main(void)
    {
        ...
        if( nullptr != mrb )
        {
            ...
            mrb_define_module_function(mrb, cppregexp_class, "reg_match",   mrb_cppregexp_reg_match,   MRB_ARGS_ANY());
            mrb_define_module_function(mrb, cppregexp_class, "reg_grep",    mrb_cppregexp_reg_grep,    MRB_ARGS_ANY());
            mrb_define_module_function(mrb, cppregexp_class, "reg_replace", mrb_cppregexp_reg_replace, MRB_ARGS_ANY());
            mrb_define_module_function(mrb, cppregexp_class, "reg_split",   mrb_cppregexp_reg_split,   MRB_ARGS_ANY());
            mrb_define_module_function(mrb, cppregexp_class, "reg_select",  mrb_cppregexp_reg_select,  MRB_ARGS_ANY());
...
mrb_value mrb_bedit_write(mrb_state * mrb, mrb_value self)
{
    ...
    if(nullptr != bedit)
    {
        ...
        case 2:
            ...
            else if(  (MRB_TT_STRING  == mrb_type(argv[0]))
        ...
        if(nullptr != data_ptr)
        {
            ...
            auto wsize = bedit->write(address, data, size);
...
mrb_value mrb_bedit_pos(mrb_state * mrb, mrb_value self)
{
    ...
    if(nullptr != bedit)
    {
        ...
        if((1 == argc) && (MRB_TT_INTEGER == mrb_type(argv[0])))
        {
            bedit->set_pos(mrb_integer(argv[0]));
        }
...
static mrb_value mrb_cppregexp_reg_match(mrb_state* mrb, mrb_value self)
{
    ...
    if(2 <= argc)
    {
        auto type = mrb_type(argv[0]);
        if((MRB_TT_STRING == type) || (MRB_TT_ARRAY == type))
        {
            ...
            for(auto idx = 1; idx < argc; idx ++)
            {
                ...
                {
                case MRB_TT_STRING:
                    text.push_back(std::string(RSTR_PTR(mrb_str_ptr(argv[idx]))));
                    break;
                case MRB_TT_ARRAY:
                    while(!mrb_nil_p(item = mrb_ary_shift(mrb, argv[idx])))
                    {
                        if(MRB_TT_STRING == mrb_type(item))
                        {
                            text.push_back(std::string(RSTR_PTR(mrb_str_ptr(item))));
                        }
                    }
                    break;
                default:
            ...
            {
            case MRB_TT_STRING:
                if(reg.match(std::string(RSTR_PTR(mrb_str_ptr(argv[0])))))
                {
                    return mrb_bool_value(true);
                }
                break;
...
static mrb_value mrb_cppregexp_reg_grep(mrb_state* mrb, mrb_value self)
{
    ...
    if(2 <= argc)
    {
        ...
        {
        case MRB_TT_STRING:
            text.push_back(std::string(RSTR_PTR(mrb_str_ptr(argv[0]))));
            break;
        case MRB_TT_ARRAY:
            while(!mrb_nil_p(item = mrb_ary_shift(mrb, argv[0])))
            {
        ...
        {
            mrb_ary_push(mrb, arry, mrb_str_new_cstr(mrb, item.c_str()));
        }
    }
    return arry;
}
...
mrb_value mrb_cppregexp_match(mrb_state * mrb, mrb_value self)
{
    ...
    mrb_int argc; mrb_value * argv;
    mrb_get_args(mrb, "*", &argv, &argc);
    if((nullptr != reg) && (1 == argc) && (MRB_TT_STRING == mrb_type(argv[0])))
    {
...
mrb_value mrb_cppregexp_replace(mrb_state * mrb, mrb_value self)
{
    ...
    mrb_int argc; mrb_value * argv;
    mrb_get_args(mrb, "*", &argv, &argc);
    if((nullptr != reg) && (2 <= argc))
    {
        const char * rep = RSTR_PTR(mrb_str_ptr(argv[0]));
        std::list<std::string> text;
        for(auto idx = 1; idx < argc; idx ++)
        {
            mrb_value item;
...
uint32_t BinaryControl::write(mrb_int address, std::string & data, mrb_int size)
{
    auto wlen = 0;
    std::vector<unsigned char> bin(data.size()/2);
    ...
    std::memcpy(&(this->data[address]), &(bin[0]), size);
    return size;
}

uint32_t BinaryControl::write_big(mrb_int address, std::string & data, mrb_int size)
{
    auto wlen = 0;
    std::vector<unsigned char> bin(data.size()/2);
    const unsigned char * str = reinterpret_cast<const unsigned char *>(data.c_str());
    ...
    {
        this->data[address + (top - idx)] = bin[idx];
    }
    return size;
}
...
bool BinaryControl::get(mrb_state * mrb, uint32_t address, std::string & format_, mrb_value & array)
{
    ...
    {
        Byte temp;
        temp.buff[0] = data[address];
        mrb_ary_push(mrb, array, mrb_int_value(mrb, temp.val));
        address += 1;
    };
    auto act_b = [&]()
    {
        Byte temp;
        temp.buff[0] = data[address];
        mrb_ary_push(mrb, array, mrb_int_value(mrb, temp.data));
        address += 1;
    };
    auto act_s = [&]()
    {
        Word temp;
        std::memcpy(&(temp.buff[0]), &(data[address]), 2);
        mrb_ary_push(mrb, array, mrb_int_value(mrb, temp.int16));
        address += 2;
    };
    auto act_w = [&]()
    {
        Word temp;
        std::memcpy(&(temp.buff[0]), &(data[address]), 2);
        mrb_ary_push(mrb, array, mrb_int_value(mrb, temp.uint16));
        address += 2;
    };
    auto act_i = [&]()
    {
        DWord temp;
        std::memcpy(&(temp.buff[0]), &(data[address]), 4);
        mrb_ary_push(mrb, array, mrb_int_value(mrb, temp.int32));
        address += 4;
    };
    auto act_d = [&]()
    {
        DWord temp;
        std::memcpy(&(temp.buff[0]), &(data[address]), 4);
        mrb_ary_push(mrb, array, mrb_int_value(mrb, temp.uint32));
        address += 4;
    };
    auto act_f = [&]()
    {
        DWord temp;
        std::memcpy(&(temp.buff[0]), &(data[address]), 4);
        mrb_ary_push(mrb, array, mrb_float_value(mrb, temp.value));
        address += 4;
    };
    auto act_S = [&]()
    {
        ...
        {
            temp.buff[idx] = data[address + (1-idx)];
        }
        mrb_ary_push(mrb, array, mrb_int_value(mrb, temp.int16));
        address += 2;
    };
    ...
    auto act_I = [&]()
    {
        ...
        {
            temp.buff[idx] = data[address + (3-idx)];
        }
        mrb_ary_push(mrb, array, mrb_int_value(mrb, temp.int32));
        address += 4;
    };
    ...
    std::map<char, std::function<void()>> act2 =
    {
        {'c', act_c},
        {'b', act_b},
        {'s', act_s}, {'S', act_S},
    ...
    auto act_a = [&]()
    {
        ...
        {
            if(address < this->size()) { size = this->size() - address; }
            else { size = 0; }
        }
        char str[size + 1];
        str[size] = '\0';
    ...
    for(auto ch: format)
    {
        ...
        {
            if(0 < act.count(state)) { (act[state])(); }
            else { }
            num.clear();
            state = ' ';
            if(0 < act2.count(ch))    { (act2[ch])(); }
            else
            {
                static const char check[] = "aAhH";
                if(nullptr != strchr(check, ch)) { state = ch; }
                else { }
            }
        }
    }
...
mrb_int BinaryControl::set(mrb_state * mrb, uint32_t address, std::string & format_, mrb_value & array)
{
    ...
    auto act_f = [&]()
    {
        ...
        if(!mrb_nil_p(item))
        {
            ...
            {
            case MRB_TT_FLOAT:
                temp.value = static_cast<float>(mrb_float(item));
                break;
            case MRB_TT_INTEGER:
                temp.value = static_cast<float>(mrb_integer(item));
                break;
            case MRB_TT_STRING:
                temp.value  = stof(std::string(RSTR_PTR(mrb_str_ptr(item))));
                break;
            default:
                len = 0;
                break;
    ...
    auto act_h = [&]()
    {
        ...
        if(MRB_TT_STRING == mrb_type(item))
        {
            ...
            auto w_size = write(address, data, (data.size() / 2));
    ...
    auto act_H = [&]()
    {
        ...
        if(MRB_TT_STRING == mrb_type(item))
        {
            ...
            auto sz = (data.size() / 2);
            if(this->size() < address+sz) { this->resize(address+sz); }
            auto w_size = write_big(address, data, sz);
            address += w_size; size += w_size;
        }
    };
...
std::list<std::string> CppRegexp::split(std::string & src)
{
    std::list<std::string> result;
